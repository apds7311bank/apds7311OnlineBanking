...
/*const createAdminUsers = async () => {
  const admins = [
    {  username: 'admin1', password: 'Abcd@1234', role: 'admin' },
    {  username: 'admin2', password: 'XyZ987@q', role: 'admin' }
  ];
...
const adminUsernameRegex = /^admin[0-9]+$/;

createAdminUsers().then(() => {
  console.log('Admin users setup completed.');
  mongoose.connection.close(); // Close the connection if running as a script
}).catch((err) => {
